...
To import the freedom folder to the **Idea IntelliJ** tool:
 ...
 - If the freedom folder wasn't compiled before, go ahead and 'make verilog' for the first time (no need to 'make mcs' though). This act is for download the dependencies for the tool (the **Idea IntelliJ** has trouble with dependencies download)
 ...
 - After the compile, go to the ***sbt*** tab on the MOST RIGHT EDGE and hit the reload button, it will re-sync the second time
